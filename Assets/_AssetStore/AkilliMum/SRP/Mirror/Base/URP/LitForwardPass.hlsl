...
float Shockwave(float3 position)
{
    float result, progress, len, smooth, smooth2, magnification;
    half edge1, edge2;
    float progressMult = 1 + _WavesDistance * (_WavesCount - 1);
    half lengthThreshold = 0.1;

    for(int i = 0; i < _WavesCount; i++)
    {
        progress = _Progress * progressMult - _WavesDistance * i;
        progress = clamp(progress, 0, 1);// min(_Progress, 1 + i * _WavesDistance);
        progress = frac(progress);
        edge1 = progress - _Size;
        edge2 = progress + _Size;
        len = length(position.xz) + lengthThreshold;
        smooth = smoothstep(edge1, edge2, len);
        smooth *= 1 - smooth;

        // Wave height
        smooth2 = smoothstep(0, 1, progress);
        smooth2 = (1 - smooth2) * (smooth2 - 0.01);
        magnification = (len * 0.4 + 0.6);
        magnification *= _WaveActivated * _Magnification * smooth2;

        result += magnification * smooth;
    }

    return result;
}

float2 uvShockwave(float2 screenUV, float2 uv)
{
    float2 centerUV = uv - _FocalPoint;
    float progress, fracProgress, smooth, smooth2;
    half edge1, edge2;
    float2 result, normalized;
    float progressMult = 1 + _WavesDistance * (_WavesCount - 1);
    half lengthThreshold = 0.05;

    for(int i = 0; i < _WavesCount; i++)
    {
        progress = _Progress * progressMult - _WavesDistance * i;
        progress = clamp(progress, 0, 1);
        fracProgress = frac(progress);
        progress = fracProgress / 2.0;
        edge1 = fracProgress - _Size;
        edge2 = fracProgress + _Size;
        smooth = smoothstep(edge1, edge2, length(centerUV) + lengthThreshold);
        smooth *= 1 - smooth;

        // Wave height
        smooth2 = smoothstep(0, 1, progress);
        smooth2 = (1 - smooth2) * smooth2 * _Magnification;
        normalized = normalize(centerUV) * _WaveActivated * smooth2 * smooth;

        result += normalized;
    }

    return screenUV + result;
}
...
Varyings LitPassVertex(Attributes input)
{
...
#ifdef _SHOCKWAVE
    positionOS.y += Shockwave(positionOS);
#endif
    ...
    output.distance = distance(_WorldSpaceCameraPos, mul(unity_ObjectToWorld, positionOS));
...
half TestNeighboursForAR(float2 screenUV, float eyeIndex, half alpha) {
    ...
    for (int i = 1; i <= 3; i++)
    {
        ...
        up = GetReflectionTexture(eyeIndex, /*uvShockwave(screenUV, input.uv)*/screenUV + float2(0, stepY)).rgb;
        down = GetReflectionTexture(eyeIndex, /*uvShockwave(screenUV, input.uv)*/screenUV + float2(0, -stepY)).rgb;
        left = GetReflectionTexture(eyeIndex, /*uvShockwave(screenUV, input.uv)*/screenUV + float2(-stepX, 0)).rgb;
        right = GetReflectionTexture(eyeIndex, /*uvShockwave(screenUV, input.uv)*/screenUV + float2(stepX, 0)).rgb;
...
half4 LitPassFragment(Varyings input) : SV_Target
{
    ...
    if(_EnableWave > 0)
    {
        ...
        #ifdef _SHOCKWAVE
        reflection = GetReflectionTexture(input.eyeIndex, uvShockwave(screenUV1, input.uv)) / 2 +
                     GetReflectionTexture(input.eyeIndex, uvShockwave(screenUV2, input.uv)) / 2;
        #else
        ...
        #ifdef _SHOCKWAVE
        reflection = GetReflectionTexture(input.eyeIndex, uvShockwave(screenUV, input.uv));
        #else
