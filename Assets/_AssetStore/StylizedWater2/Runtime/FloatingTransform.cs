...
namespace StylizedWater2
{
    ...
    public class FloatingTransform : MonoBehaviour
    {
        ...
        public void FixedUpdate()
        {
            ...
            {
                height = Buoyancy.SampleWaves(this.transform.position, waterObject.material, m_waterLevel - heightOffset, rollAmount, true, out normal);
            }
            else
            {
                ...
                {
                    height += Buoyancy.SampleWaves(ConvertToWorldSpace(samples[i]), waterObject.material, m_waterLevel - heightOffset, rollAmount, true, out normal);
                    avgNormal += normal;
                }
        ...
        private IEnumerator Interpolation()
        {
            ...
            while(t < 1f)
            {
                ...
                if(rollAmount > 0f)
                {
                    transform.up = Vector3.Lerp(startUp, normal, t);
                    transform.rotation *= _startRot;
                }
